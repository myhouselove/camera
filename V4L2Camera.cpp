...
int V4L2Camera::Open (const char *device)
{
    ...
    videoIn->formatIn = V4L2_PIX_FMT_YUV422P;

    videoIn->format.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    videoIn->format.fmt.pix.width = 640;
    videoIn->format.fmt.pix.height = 480;
    videoIn->format.fmt.pix.pixelformat = V4L2_PIX_FMT_YUV422P;
...
int V4L2Camera::getUVCData(unsigned short* ref){
	...
	if(ref != NULL){
		memcpy(ref,videoIn->mem[videoIn->buf.index],videoIn->buf.length);
		//ALOGE("INDEX = %d\n",videoIn->buf.index);
	}
...
{
    ptr = new unsigned short[videoIn->buf.length];
